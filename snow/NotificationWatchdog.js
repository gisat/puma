            ...
            await notificationWatchdog.getGroupedByScope(pool)
                .then(async (rows) => {
                    for (let row of rows) {
                        ...
                        let configuration_name = `${area_value}/${date_from}-${date_to}/${sensorsString}${compositeString}`;

                        if (done && created && updated && Date.now() - updated >= 15000) {
                            await pgUsers.byId(owner)
                                .then((user) => {
                                    if (
                                        updated - created <= 30000
                                        ...
                                        return snowConfigurationTable.insert({
                                            ...
                                            <p>You can view the results in the 'Saved configurations' of your user account.</p>
                                            <p>This email has been automatically generated by the Snow Portal</p>`
                                            );
                                        }).then(() => {
                                            return notificationWatchdog.setAsNotified(ids);
                                        });
