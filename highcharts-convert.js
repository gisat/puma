    ...
    if (args.length < 1) {
        ...
        if (input.split('.').pop() === 'json') {
            ...
            for (var i = 0; i < options.length; i++) {
                optionsStr = JSON.stringify(options[i]);
                ...
                try {
                    ...
                    if (outputExtension === 'svg') {
                        //var outName = output.replace('%gid%', options[i].gid);
                        //console.log(outName);
                        var replacedSvg = svg.html.replace(/<svg /, '<svg xmlns:xlink="http://www.w3.org/1999/xlink" ')
                                .replace(/ href=/g, ' xlink:href=').replace(/<\/svg>.*?$/, '</svg>')
                        options[i].html = replacedSvg
                        //svgFile = fs.open(outName, "w");
            ...
            if (outputExtension === 'svg') {
                ...
                for (var i=0;i<options.length;i++) {
                    ...
                    map[option.gid] = option.html;
                }
                console.log(JSON.stringify(map));
            }
            if (outputExtension === 'svg' || window.imagesLoaded) {
                phantom.exit();
            }
