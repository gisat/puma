...
class PgScopeScenarioCaseRelations extends PgCollection {
	...
	create(objects) {
		...
		objects.forEach((object) => {
			...
			promises.push(
				this._pgPool.query(`INSERT INTO "${this._pgMetadataSchema}"."${PgScopeScenarioCaseRelations.tableName()}" (${columns.join(', ')}) VALUES (${values.join(', ')});`)
			);
		});
	...
	getFiltered(filter) {
		...
		return this._pgPool.query(`SELECT * FROM "${this._pgMetadataSchema}"."${PgScopeScenarioCaseRelations.tableName()}";`)
			.then((queryResult) => {
	...
	update(updates) {
		...
		updates.forEach((update) => {
			...
			if (scenario_case_id && (scope_ids || scope_ids === null)) {
				promises.push(
					this._pgPool.query(`DELETE FROM "${this._pgMetadataSchema}"."${PgScopeScenarioCaseRelations.tableName()}" WHERE "scenario_case_id" = ${scenario_case_id};`)
						.then(() => {
							if (scope_ids) {
