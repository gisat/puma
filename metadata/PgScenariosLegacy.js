...
class PgScenariosLegacy {
	...
	createOne(object, payloadData, user, extra) {
		...
		return this._pgPool
			.query(
				`INSERT INTO "${this._pgSchema}"."${PgScenariosLegacy.tableName()}" (${columns.join(', ')}) VALUES (${_.map(values, (value, index) => {
					return `$${index + 1}`
				}).join(', ')}) RETURNING id;`,
				values
			)
			...
			.then((id) => {
				...
				promises.push(this._pgPermissions.add(user.id, PgScenariosLegacy.tableName(), id, Permission.CREATE));
				promises.push(this._pgPermissions.add(user.id, PgScenariosLegacy.tableName(), id, Permission.READ));
				promises.push(this._pgPermissions.add(user.id, PgScenariosLegacy.tableName(), id, Permission.UPDATE));
				promises.push(this._pgPermissions.add(user.id, PgScenariosLegacy.tableName(), id, Permission.DELETE));
											...
											if (_.isString(scenarioCaseId)) {
												if (scenario_cases) {
													...
													if (scenario_case && !scenario_case.id) {
														return this._pgScenarioCases.create(payloadData, user, extra)
															.then((results) => {
																if (results.length) {
	...
	get(filter) {
		...
		pagingQuery.push(`FROM "${this._pgSchema}"."${PgScenariosLegacy.tableName()}" AS a`);
		...
		query.push(`FROM "${this._pgSchema}"."${PgScenariosLegacy.tableName()}" AS a`);
			...
			scenarios.forEach((update) => {
				...
				if (data) {
					...
					if (keys.length) {
						...
						if (id) {
							promises.push(
								this._pgPool.query(`UPDATE "${this._pgSchema}"."${PgScenariosLegacy.tableName()}" SET ${sets.join(', ')} WHERE id = ${id}`)
									.then((result) => {
										return {
											id: id
										}
							...
							promises.push(
								this.createOne(update, payloadData, user, extra)
							);
						}
					}
				} else {
	...
	deleteOne(scenarioId) {
		...
		return this._pgPool.query(
			`DELETE FROM "${this._pgSchema}"."${PgScenariosLegacy.tableName()}" WHERE id = ${scenarioId}`
		).then((result) => {
			if (result.rowCount) {
				status.deleted = true;
			}
			...
			return this._pgPermissions.removeAllForResource(PgScenariosLegacy.tableName(), scenarioId);
		}).then(() => {
			return status;
		});
	}
...
module.exports = PgScenariosLegacy;