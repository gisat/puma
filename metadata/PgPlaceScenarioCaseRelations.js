...
class PgPlaceScenarioCaseRelations extends PgCollection {
	...
	create(objects) {
		...
		objects.forEach((object) => {
			...
			promises.push(
				this._pgPool.query(`INSERT INTO "${this._pgMetadataSchema}"."${PgPlaceScenarioCaseRelations.tableName()}" (${columns.join(', ')}) VALUES (${values.join(', ')});`)
			);
		});
	...
	getFiltered(filter) {
		...
		return this._pgPool.query(`SELECT * FROM "${this._pgMetadataSchema}"."${PgPlaceScenarioCaseRelations.tableName()}";`)
			.then((queryResult) => {
	...
	update(updates) {
		...
		updates.forEach((update) => {
			...
			if(scenario_case_id && (place_ids || place_ids === null)) {
				promises.push(
					this._pgPool.query(`DELETE FROM "${this._pgMetadataSchema}"."${PgPlaceScenarioCaseRelations.tableName()}" WHERE "scenario_case_id" = ${scenario_case_id};`)
						.then(() => {
							if(place_ids) {
