...
class GeneratedImage {
	...
	generate() {
		...
		return this.path().then(function(path){
			...
			pathOfResult = path;
			logger.info('GeneratedImage#generate Start process. Url: ', self.url, ' Path: ', path, ' Whole result: ', `http://api.screenshotmachine.com/?key=a647a7&dimension=1024x768&format=png&timeout=5000&url=${config.printUrl}%3Fid%3D${self.id}%26print`);
			return superagent.get(`http://api.screenshotmachine.com/?key=a647a7&dimension=1024x768&format=png&timeout=5000&url=${config.printUrl}%3Fid%3D${self.id}%26print`);
