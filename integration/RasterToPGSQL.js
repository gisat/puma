...
class RasterToPGSQL {
    ...
    import(layer) {
        return new Promise((resolve, reject) => {
            child_process.exec(`
                raster2pgsql -d -I -C -k -F -t 512x512 ${layer.directory}${layer.files[0]} public."${layer.name}" | PGPASSWORD=${this._pgPassword} psql -h ${this._pgHost} -U ${this._pgUser} ${this._pgDatabase}
            `, (error, stdout, stderr) => {
                if (error) {
                    reject(error);
                } else {
