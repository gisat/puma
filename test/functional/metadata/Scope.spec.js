...
let DatasetController = require('../../../routes/DatasetController');
...
describe('Scope', () => {
	...
	beforeEach(function (done) {
		...
		new DatasetController(app, pool, commonSchema);
		...
		it('returns all', done => {
			...
			supertest(app)
				...
				.then((response) => {
					...
					should(response.body.data[1].permissions.user.length).be.exactly(1);
					should(response.body.data[2].permissions.group.length).be.exactly(1);
					done();
				}).catch(err => {
				done(err);
			});
