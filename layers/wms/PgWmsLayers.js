...
class PgWmsLayers {
	 ...
	 * @param layer.getDate {Boolean} True if this layer allows date line and time movements.
	...
	add(layer, userId) {
		...
		return this._pool.query(`
			INSERT INTO ${this.schema}.${PgWmsLayers.tableName()} (name, layer, url, ${scope} created, created_by, changed, changed_by, custom, get_date) VALUES ('${layer.name}','${layer.layer}','${layer.url}',${scopeValue} '${time}', ${userId}, '${time}', ${userId}, '${layer.custom}', ${layer.getDate}) RETURNING id;`).then(result => {
			id = result.rows[0].id;
	...
	update(layer, userId) {
		...
		logger.info('PgWmsLayer#update Layer: ', layer, ' SQL: ', `UPDATE ${this.schema}.${PgWmsLayers.tableName()} SET name = '${layer.name}', url = '${layer.url}', layer='${layer.layer}', ${scopeSql} changed='${time}', changed_by=${userId}, get_date=${layer.getDate} where id = ${layer.id}`);
		return this._pool.query(`UPDATE ${this.schema}.${PgWmsLayers.tableName()} SET name = '${layer.name}', url = '${layer.url}', layer='${layer.layer}', ${scopeSql} changed='${time}', changed_by=${userId}, custom='${layer.custom}', get_date=${layer.getDate} where id = ${layer.id}`).then(() => {
