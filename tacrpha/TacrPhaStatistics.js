...
class TacrPhaStatistics {
    ...
    insert(ip, keywords, result){
        ...
        var sql = `INSERT INTO ${config.pgSchema.data}.` + this._table + ` ` +
    ...
    getSearchStringFrequency(type){
        ...
        var sql = `SELECT keywords, COUNT(keywords) as num FROM ${config.pgSchema.data}.` + this._table + where + ` GROUP BY keywords ORDER by num DESC;`;
