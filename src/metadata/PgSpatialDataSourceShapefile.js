...
class PgSpatialDataSourceShapefile extends PgCollection {
	constructor(pgPool, pgSchema) {
		super(pgPool, pgSchema, 'PgDataSourceShapefile');
	}
	...
	create(objects) {
		...
		objects.forEach((object) => {
			...
			promises.push(
				this._pgPool.query(`INSERT INTO "${this._pgMetadataSchema}"."${PgSpatialDataSourceShapefile.tableName()}" (${columns.join(', ')}) VALUES (${values.join(', ')}) RETURNING id;`)
					.then((queryResult) => {
	...
	get(filter) {
		...
		pagingQuery.push(`FROM "${this._pgMetadataSchema}"."${PgSpatialDataSourceShapefile.tableName()}" AS a`);

		let query = [];
		query.push(`SELECT *`);
		query.push(`FROM "${this._pgMetadataSchema}"."${PgSpatialDataSourceShapefile.tableName()}" AS a`);
	...
	update(updates) {
		...
		updates.forEach((record) => {
			...
			if (keys.length) {
				...
				promises.push(
					this._pgPool.query(`UPDATE "${this._pgMetadataSchema}"."${PgSpatialDataSourceShapefile.tableName()}" SET ${sets.join(', ')} WHERE id = ${id}`)
						.then((result) => {
							if (result.rowCount) {
								return this.get({id: id, unlimited: true});
							}
	...
	delete(id) {
		return new Promise((resolve, reject) => {
			id = Number(id);
			if (!_.isNaN(id)) {
				this._pgPool.query(`DELETE FROM "${this._pgMetadataSchema}"."${PgSpatialDataSourceShapefile.tableName()}" WHERE id = ${Number(id)};`)
					.then((result) => {
						if (result.rowCount) {
							resolve();
						} else {
	...
	static tableName() {
		return `spatial_data_source_shapefile`;
	}
...
module.exports = PgSpatialDataSourceShapefile;