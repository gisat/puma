...
function getLayers(params, req, res, callback) {
    ...
    var client = conn.getPgDataDb();
    var layers = `SELECT tablename FROM pg_tables WHERE tableowner = 'geonode' AND schemaname = 'public';`;

    client.query(layers, [], function (error, response) {
        var pgLayers = [];
        var pgLayersFilter = [];

        for (var row of response.rows) {
            pgLayersFilter.push(new RegExp(`^.*:${row.tablename}$`));
            pgLayers.push({
                name: row.tablename,
                referenced: false,
                isGeonode: false
        ...
        var filter = {layer: {$in: pgLayersFilter}};
        crud.read('layerref', filter, function (err, result) {
            ...
            var mongoLayerrefs = [];
            for (var mongoLayerref of result) {
                var mongoLayerName = new RegExp(`^.*:(.*)$`).exec(mongoLayerref.layer)[1];
                if (!mongoLayerrefs.includes(mongoLayerName)) {
                    mongoLayerrefs.push(mongoLayerName);
                }
            }

            for (var pgLayerIndex in pgLayers) {
                if (mongoLayerrefs.includes(pgLayers[pgLayerIndex].name)) {
                    pgLayers[pgLayerIndex].referenced = true;
                }
            }

            conn.request(options, null, function (error, output, resl) {
                if (error) {
                    logger.error(`api/layers.js #getLayers => Request options: ${options}, Error: ${error}`);
                ...
                gnClient.query(ownedLayers, [], function (error, response) {
                    if (error) {
                        logger.error(`api/layers.js #getLayers => Query: ${ownedLayers}, Error: ${error}`);
                        return callback(error);
                    }
